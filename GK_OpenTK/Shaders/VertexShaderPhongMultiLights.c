...
void main()
{
	...
	Normal_cameraspace = (view *transpose(inverse( model)) * vec4(normal_modelspace, 0)).xyz; // Only correct if ModelMatrix does not scale the model ! Use its inverse transpose if not.
